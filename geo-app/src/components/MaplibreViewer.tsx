...
const MaplibreViewer = ({ data, activeLayers, onEntityClick, flyToLocation, selectedEntity, onMouseCoords, onRightClick, regionDossier, regionDossierLoading, onViewStateChange, measureMode, onMeasureClick, measurePoints }: any) => {
    ...
    // ── NEW DATA SOURCE GeoJSON builders ──

    // NASA FIRMS active wildfires — orange/red heat dots
    const wildfiresGeoJSON = useMemo(() => {
        if (!activeLayers.wildfires || !data?.wildfires?.length) return null;
        return {
            type: 'FeatureCollection' as const,
            features: data.wildfires.map((f: any, i: number) => ({
                type: 'Feature' as const,
                properties: {
                    id: i,
                    type: 'wildfire',
    ...
    }, [activeLayers.wildfires, data?.wildfires]);

    // NASA EONET natural events — volcanoes, storms, icebergs
    const naturalEventsGeoJSON = useMemo(() => {
        if (!activeLayers.natural_events || !data?.natural_events?.length) return null;
        return {
            type: 'FeatureCollection' as const,
            features: data.natural_events.map((ev: any, i: number) => ({
                type: 'Feature' as const,
                properties: {
                    id: i,
                    type: 'natural_event',
    ...
    const weatherStationsGeoJSON = useMemo(() => {
        if (!activeLayers.weather_stations || !data?.weather_stations?.length) return null;
        return {
            type: 'FeatureCollection' as const,
            features: data.weather_stations.map((s: any, i: number) => ({
                type: 'Feature' as const,
                properties: {
                    id: i,
    ...
    }, [activeLayers.weather_stations, data?.weather_stations]);

    // Submarine cable landing points — blue dots
    const submarineCablesGeoJSON = useMemo(() => {
        if (!activeLayers.submarine_cables || !data?.submarine_cables?.length) return null;
        return {
            type: 'FeatureCollection' as const,
            features: data.submarine_cables.map((c: any, i: number) => ({
                type: 'Feature' as const,
                properties: {
                    id: i,
    ...
    // eBird notable sightings — green bird markers
    const ebirdGeoJSON = useMemo(() => {
        if (!activeLayers.ebird || !data?.ebird_sightings?.length) return null;
        return {
            type: 'FeatureCollection' as const,
            features: data.ebird_sightings.map((s: any, i: number) => ({
                type: 'Feature' as const,
                properties: {
                    id: i,
                    type: 'ebird',
    ...
    }, [activeLayers.ebird, data?.ebird_sightings]);

    // PurpleAir sensors — colored by AQI
    const purpleairGeoJSON = useMemo(() => {
        if (!activeLayers.purpleair || !data?.purpleair?.length) return null;
        return {
            type: 'FeatureCollection' as const,
            features: data.purpleair.map((s: any, i: number) => {
                const aqi = s.aqi || 0;
                const color = aqi <= 50 ? '#22c55e' : aqi <= 100 ? '#eab308' : aqi <= 150 ? '#f97316' : aqi <= 200 ? '#ef4444' : aqi <= 300 ? '#a855f7' : '#991b1b';
                return {
                    type: 'Feature' as const,
                    properties: {
                        id: i,
    ...
    }, [activeLayers.purpleair, data?.purpleair]);

    // Ticketmaster events — magenta event markers
    const eventsGeoJSON = useMemo(() => {
        if (!activeLayers.events || !data?.events?.length) return null;
        return {
            type: 'FeatureCollection' as const,
            features: data.events.map((ev: any, i: number) => ({
                type: 'Feature' as const,
                properties: {
                    id: i,
                    type: 'event',
    ...
    }, [activeLayers.events, data?.events]);

    // Global Fishing Watch — fishing activity dots
    const fishingGeoJSON = useMemo(() => {
        if (!activeLayers.fishing || !data?.fishing_activity?.length) return null;
        return {
            type: 'FeatureCollection' as const,
            features: data.fishing_activity.filter((f: any) => f.lat != null && f.lng != null).map((f: any, i: number) => ({
                type: 'Feature' as const,
                properties: {
                    id: i,
                    type: 'fishing',
                    flag: f.flag,
                    hours: f.hours,
                    name: `Fishing: ${f.flag} (${Math.round(f.hours)}h)`,
                },
                geometry: { type: 'Point' as const, coordinates: [f.lng, f.lat] }
            }))
        };
    }, [activeLayers.fishing, data?.fishing_activity]);

    // Load Images into the Map Style once loaded
    const onMapLoad = useCallback((e: any) => {
        const map = e.target;

        // On mobile, disable rotation but keep pinch-to-zoom
        if (isMobile) {
    ...
    const activeInteractiveLayerIds = [
        ...
        naturalEventsGeoJSON && 'natural-events-layer',
        weatherStationsGeoJSON && 'weather-stations-layer',
        submarineCablesGeoJSON && 'submarine-cables-layer',
        ebirdGeoJSON && 'ebird-layer',
        eventsGeoJSON && 'events-layer',
    ...
    return (
        <div className={`relative h-full w-full z-0 isolate ${selectedEntity && ['region_dossier', 'gdelt', 'liveuamap', 'news'].includes(selectedEntity.type) ? 'map-focus-active' : ''}`}>
            ...
            >
                ...
                {/* NASA FIRMS Wildfires — orange/red heat dots with clustering */}
                {wildfiresGeoJSON && (
                    <Source id="wildfires" type="geojson" data={wildfiresGeoJSON as any} cluster={true} clusterMaxZoom={8} clusterRadius={40}>
                        <Layer
                            id="wildfires-cluster"
                            type="circle"
                            filter={['has', 'point_count']}
                            paint={{
                                'circle-color': '#ff4500',
                                'circle-radius': ['step', ['get', 'point_count'], 10, 20, 14, 100, 18, 500, 24],
                                'circle-opacity': 0.85,
                                'circle-stroke-width': 2,
                ...
                {/* NASA EONET Natural Events — green markers */}
                {naturalEventsGeoJSON && (
                    <Source id="natural-events" type="geojson" data={naturalEventsGeoJSON as any}>
                        <Layer
                ...
                {/* Open-Meteo Weather Stations — white text labels with temp */}
                {weatherStationsGeoJSON && (
                    <Source id="weather-stations" type="geojson" data={weatherStationsGeoJSON as any}>
                        <Layer
                ...
                {/* Submarine Cable Landing Points — small blue diamonds */}
                {submarineCablesGeoJSON && (
                    <Source id="submarine-cables" type="geojson" data={submarineCablesGeoJSON as any} cluster={true} clusterMaxZoom={6} clusterRadius={30}>
                        <Layer
                            id="submarine-cables-cluster"
                            type="circle"
                            filter={['has', 'point_count']}
                            paint={{
                                'circle-color': '#6366f1',
                ...
                {/* ISS Tracker — special animated marker */}
                {activeLayers.iss && data?.iss_position && (
                    <Marker longitude={data.iss_position.lng} latitude={data.iss_position.lat} anchor="center">
                        <div className="flex flex-col items-center pointer-events-none">
                            <div className="w-8 h-8 rounded-full border-2 border-purple-400 animate-ping absolute opacity-20" />
                            <div className="w-6 h-6 rounded-full bg-purple-600 border-2 border-purple-300 shadow-[0_0_16px_rgba(168,85,247,0.7)] flex items-center justify-center">
                                <span className="text-[7px] font-mono font-bold text-white">ISS</span>
                            </div>
                            <span className="text-[8px] font-mono text-purple-300 mt-1 bg-black/60 px-1 rounded">ISS</span>
                        </div>
                    </Marker>
                )}

                {/* eBird Notable Sightings — lime green clustered */}
                {ebirdGeoJSON && (
                    <Source id="ebird" type="geojson" data={ebirdGeoJSON as any} cluster={true} clusterMaxZoom={8} clusterRadius={40}>
                        <Layer
                            id="ebird-cluster"
                            type="circle"
                            filter={['has', 'point_count']}
                ...
                {/* PurpleAir Sensors — AQI-colored dots with clustering */}
                {purpleairGeoJSON && (
                    <Source id="purpleair" type="geojson" data={purpleairGeoJSON as any} cluster={true} clusterMaxZoom={8} clusterRadius={30}>
                        <Layer
                            id="purpleair-cluster"
                            type="circle"
                            filter={['has', 'point_count']}
                            paint={{
                                'circle-color': '#eab308',
                                'circle-radius': ['step', ['get', 'point_count'], 8, 20, 12, 100, 16],
                                'circle-opacity': 0.7,
                                'circle-stroke-width': 1,
                ...
                {/* Ticketmaster Events — magenta dots */}
                {eventsGeoJSON && (
                    <Source id="events" type="geojson" data={eventsGeoJSON as any} cluster={true} clusterMaxZoom={10} clusterRadius={40}>
                        <Layer
                            id="events-cluster"
                            type="circle"
                            filter={['has', 'point_count']}
                ...
                {/* Global Fishing Watch — teal fishing activity */}
                {fishingGeoJSON && (
                    <Source id="fishing" type="geojson" data={fishingGeoJSON as any}>
                        <Layer
                            id="fishing-layer"
                            type="circle"
                            paint={{
            ...
            {/* Space Weather HUD overlay — shown when layer is active */}
            {activeLayers.space_weather && data?.space_weather?.length > 0 && (
                <div className="absolute top-2 left-1/2 -translate-x-1/2 z-10 bg-black/70 backdrop-blur-md border border-yellow-500/40 rounded-lg px-4 py-2 flex items-center gap-4 pointer-events-none">
                    {data.space_weather.filter((a: any) => a.type === 'kp_index').map((a: any, i: number) => (
                        <div key={i} className="flex items-center gap-2">
                            <span className="text-[8px] font-mono tracking-widest text-yellow-500">GEOMAG Kp</span>
                            <span className={`text-sm font-mono font-bold ${a.value >= 5 ? 'text-red-400' : a.value >= 4 ? 'text-yellow-400' : 'text-green-400'}`}>
                                {a.value}
                            </span>
                            <span className={`text-[8px] font-mono tracking-widest uppercase ${a.level === 'quiet' ? 'text-green-500' : a.level === 'minor' ? 'text-yellow-500' : 'text-red-500'}`}>
                                {a.level}
                            </span>
                        </div>
                    ))}
                    {data.space_weather.filter((a: any) => a.type === 'solar_flare' && a.max_class).slice(0, 3).map((a: any, i: number) => (
                        <div key={`flare-${i}`} className="flex items-center gap-1">
                            <span className="text-[8px] font-mono tracking-widest text-orange-500">FLARE</span>
                            <span className="text-[10px] font-mono font-bold text-orange-400">{a.max_class}</span>
                        </div>
                    ))}
                </div>
            )}
