...
import { Plane, AlertTriangle, Activity, Satellite, Cctv, ChevronDown, ChevronUp, Ship, Eye, Anchor, Settings, Sun, BookOpen, Radio, Flame, CloudSun, Globe, Zap, Cable, Power, Bird, Wind, Ticket, Fish } from "lucide-react";
...
const WorldviewLeftPanel = React.memo(function WorldviewLeftPanel({ data, activeLayers, setActiveLayers, onSettingsClick, onLegendClick }: { data: any; activeLayers: any; setActiveLayers: any; onSettingsClick?: () => void; onLegendClick?: () => void }) {
    ...
    const layers = [
        ...
        // New data sources
        { id: "wildfires", name: "Active Wildfires", source: "NASA FIRMS", count: data?.wildfires?.length || 0, icon: Flame },
        { id: "natural_events", name: "Natural Events", source: "NASA EONET", count: data?.natural_events?.length || 0, icon: Globe },
        { id: "weather_stations", name: "Weather Stations", source: "Open-Meteo", count: data?.weather_stations?.length || 0, icon: CloudSun },
        { id: "iss", name: "ISS Tracker", source: "Open Notify", count: data?.iss_position ? 1 : 0, icon: Satellite },
        { id: "space_weather", name: "Space Weather", source: "NOAA SWPC", count: data?.space_weather?.length || 0, icon: Zap },
        { id: "submarine_cables", name: "Submarine Cables", source: "TeleGeography", count: data?.submarine_cables?.length || 0, icon: Cable },
        // API-key data sources
        { id: "ebird", name: "Bird Sightings", source: "eBird / Cornell", count: data?.ebird_sightings?.length || 0, icon: Bird },
        { id: "purpleair", name: "Air Quality (PM2.5)", source: "PurpleAir", count: data?.purpleair?.length || 0, icon: Wind },
        { id: "events", name: "Live Events", source: "Ticketmaster", count: data?.events?.length || 0, icon: Ticket },
        { id: "fishing", name: "Fishing Activity", source: "Global Fishing Watch", count: data?.fishing_activity?.length || 0, icon: Fish },
    ];
    ...
    // All On/Off toggle
    const allOn = useMemo(() => Object.values(activeLayers).every(Boolean), [activeLayers]);
    const toggleAll = () => {
        const newState = !allOn;
        setActiveLayers((prev: any) => {
            const next: any = {};
            for (const key in prev) next[key] = newState;
            return next;
        });
    };
                ...
                <div className="flex items-center gap-3">
                    ...
                    {onSettingsClick && (
                        <button
                            ...
                            className="w-11 h-11 md:w-7 md:h-7 rounded-lg border border-gray-700 hover:border-cyan-500/50 flex items-center justify-center text-gray-500 hover:text-cyan-400 transition-all hover:bg-cyan-950/20 group"
                    ...
                    {onLegendClick && (
                        <button
                            ...
                            className="h-11 md:h-7 px-3 md:px-2 rounded-lg border border-gray-700 hover:border-cyan-500/50 flex items-center justify-center gap-1 text-gray-500 hover:text-cyan-400 transition-all hover:bg-cyan-950/20"
            ...
            <div className="bg-black/40 backdrop-blur-md border border-gray-800 rounded-xl pointer-events-auto shadow-[0_4px_30px_rgba(0,0,0,0.5)] flex flex-col relative overflow-hidden max-h-full">
                ...
                <AnimatePresence>
                    {!isMinimized && (
                        ...
                        >
                            {/* QUICK TOGGLES — high-volume layers */}
                            <div className="px-4 pt-3 pb-1 flex gap-2">
                                <button
                                    onClick={() => setActiveLayers((prev: any) => ({ ...prev, satellites: !prev.satellites }))}
                                    className={`flex-1 py-2 min-h-[44px] md:min-h-0 md:py-1.5 rounded-lg border text-[9px] font-mono tracking-widest font-bold transition-all flex items-center justify-center gap-1.5 ${
                                        activeLayers.satellites
                                            ? 'border-purple-500/50 text-purple-400 bg-purple-950/30 hover:bg-purple-950/50'
                                            : 'border-gray-700 text-gray-500 bg-transparent hover:border-gray-600 hover:text-gray-400'
                                    }`}
                                >
                                    <Satellite size={12} /> SATELLITES
                                </button>
                                <button
                                    onClick={() => setActiveLayers((prev: any) => ({ ...prev, flights: !prev.flights }))}
                                    className={`flex-1 py-2 min-h-[44px] md:min-h-0 md:py-1.5 rounded-lg border text-[9px] font-mono tracking-widest font-bold transition-all flex items-center justify-center gap-1.5 ${
                                        activeLayers.flights
                                            ? 'border-amber-500/50 text-amber-400 bg-amber-950/30 hover:bg-amber-950/50'
                                            : 'border-gray-700 text-gray-500 bg-transparent hover:border-gray-600 hover:text-gray-400'
                                    }`}
                                >
                            ...
                            <div className="px-4 pt-1 pb-1">
                                <button
                                    onClick={toggleAll}
                                    className={`w-full py-2 min-h-[44px] md:min-h-0 md:py-1.5 rounded-lg border text-[9px] font-mono tracking-widest font-bold transition-all ${
                                        allOn
                                            ? 'border-cyan-500/50 text-cyan-400 bg-cyan-950/30 hover:bg-cyan-950/50'
                                            : 'border-gray-700 text-gray-500 bg-transparent hover:border-gray-600 hover:text-gray-400'
                                    }`}
                                >
                            ...
                            <div className="flex flex-col gap-4 md:gap-6 p-4 pt-2 pb-6">
                                {layers.map((layer, idx) => {
                                    ...
                                    return (
                                        <div
                                            key={idx}
                                            className="flex items-start justify-between group cursor-pointer min-h-[44px] md:min-h-0 items-center md:items-start"
