...
_ws_running = False
_MAX_VESSELS = 5000  # Hard cap on tracked vessels to limit memory
            ...
            for raw_msg in iter(process.stdout.readline, ''):
                ...
                with _vessels_lock:
                    if mmsi not in _vessels:
                        # Enforce hard cap — evict oldest vessels when at limit
                        if len(_vessels) >= _MAX_VESSELS:
                            oldest = min(_vessels, key=lambda k: _vessels[k].get("_updated", 0))
                            del _vessels[oldest]
