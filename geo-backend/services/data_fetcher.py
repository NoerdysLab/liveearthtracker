...
latest_data = {
    ...
    "gdelt": [],
    "liveuamap": [],
    # New data sources (no API key required)
    "wildfires": [],
    "natural_events": [],
    "weather_stations": [],
    "iss_position": None,
...
def fetch_flights():
    ...
    def _accumulate_trail(f, now_ts, check_route=True):
        ...
        if trail_data['points'] and trail_data['points'][-1][0] == point[0] and trail_data['points'][-1][1] == point[1]:
            ...
            trail_data['last_seen'] = now_ts
        if len(trail_data['points']) > 100:
            trail_data['points'] = trail_data['points'][-100:]
...
_MAX_TRACKED_TRAILS = 800  # Global cap on number of aircraft trails in memory
...
def fetch_wildfires():
    """NASA FIRMS — active fire/hotspot detections from MODIS & VIIRS satellites."""
    fires = []
    try:
        # FIRMS provides a CSV of active fires in the last 24h (MODIS C6.1)
        url = "https://firms.modaps.eosdis.nasa.gov/api/area/csv/FIRMS_OPEN/MODIS_NRT/world/1"
        ...
        if res.status_code == 200:
            ...
            for row in reader:
                try:
                    ...
                    # Only include medium/high confidence detections
                    if confidence in ("nominal", "low"):
                        continue
                    fires.append({
                        "lat": lat,
                        "lng": lng,
                    ...
                    continue
        logger.info(f"Fetched {len(fires)} active fire detections from NASA FIRMS")
    except Exception as e:
        logger.error(f"Error fetching wildfires: {e}")
    latest_data["wildfires"] = fires[:2000]  # Cap to prevent huge payloads


def fetch_natural_events():
    """NASA EONET — natural events (volcanoes, storms, icebergs, wildfires, etc.)."""
    events = []
    try:
        ...
        logger.info(f"Fetched {len(events)} natural events from NASA EONET")
    except Exception as e:
        logger.error(f"Error fetching natural events: {e}")
    latest_data["natural_events"] = events


def fetch_weather_stations():
    """Open-Meteo — weather conditions at major world cities (no API key needed)."""
    stations = []
    # Sample 40 cities globally for weather overlay
    cities = [
        (40.71, -74.01, "New York"), (51.51, -0.13, "London"), (48.86, 2.35, "Paris"),
        (35.68, 139.69, "Tokyo"), (55.76, 37.62, "Moscow"), (-33.87, 151.21, "Sydney"),
        (39.91, 116.40, "Beijing"), (28.61, 77.21, "Delhi"), (-23.55, -46.63, "São Paulo"),
        (30.04, 31.24, "Cairo"), (19.43, -99.13, "Mexico City"), (37.57, 126.98, "Seoul"),
        (52.52, 13.41, "Berlin"), (41.39, 2.17, "Barcelona"), (59.33, 18.07, "Stockholm"),
        (1.35, 103.82, "Singapore"), (-1.29, 36.82, "Nairobi"), (33.69, 73.04, "Islamabad"),
        (23.81, 90.41, "Dhaka"), (13.76, 100.50, "Bangkok"), (6.52, 3.38, "Lagos"),
        (-34.60, -58.38, "Buenos Aires"), (35.69, 51.39, "Tehran"), (31.23, 121.47, "Shanghai"),
        (43.65, -79.38, "Toronto"), (34.05, -118.24, "Los Angeles"), (25.20, 55.27, "Dubai"),
        (14.60, 120.98, "Manila"), (21.03, 105.85, "Hanoi"), (41.01, 28.98, "Istanbul"),
        (-4.04, 39.67, "Mombasa"), (64.15, -21.94, "Reykjavik"), (-22.91, -43.17, "Rio"),
        (47.50, 19.04, "Budapest"), (50.45, 30.52, "Kyiv"), (33.44, -112.07, "Phoenix"),
        (25.79, -80.13, "Miami"), (37.78, -122.42, "San Francisco"), (60.17, 24.94, "Helsinki"),
        (38.72, -9.14, "Lisbon"),
    ]
    try:
        lats = ",".join(str(c[0]) for c in cities)
        lngs = ",".join(str(c[1]) for c in cities)
        url = (
        ...
        if res.status_code == 200:
            data = res.json()
            # Open-Meteo returns array for multi-location requests
        ...
        logger.info(f"Fetched weather for {len(stations)} stations from Open-Meteo")
    except Exception as e:
        logger.error(f"Error fetching weather stations: {e}")
    latest_data["weather_stations"] = stations
...
def fetch_iss_position():
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            latest_data["iss_position"] = {
                "lat": float(pos.get("latitude", 0)),
                "lng": float(pos.get("longitude", 0)),
                "timestamp": data.get("timestamp"),
            }
            logger.info(f"ISS at {pos.get('latitude')}, {pos.get('longitude')}")
    except Exception as e:
        logger.error(f"Error fetching ISS position: {e}")


def fetch_space_weather():
    """NOAA SWPC — space weather alerts and geomagnetic storm data."""
    alerts = []
    try:
        # Geomagnetic storm (Kp index) — current conditions
        ...
        if res.status_code == 200:
            ...
            if len(data) > 1:
                ...
                alerts.append({
                    ...
                    "value": kp,
                    "time": latest[0],
                    "level": "extreme" if kp >= 8 else "severe" if kp >= 7 else "strong" if kp >= 6 else "moderate" if kp >= 5 else "minor" if kp >= 4 else "quiet",
                })

        # Solar flare alerts
        url2 = "https://services.swpc.noaa.gov/json/goes/primary/xray-flares-latest.json"
        res2 = fetch_with_curl(url2, timeout=10)
        if res2.status_code == 200:
            flares = res2.json()
            for f in flares[:10]:
                alerts.append({
                    "type": "solar_flare",
                    "class": f.get("current_class", ""),
        ...
        logger.info(f"Fetched {len(alerts)} space weather alerts from NOAA SWPC")
    except Exception as e:
...
def fetch_carbon_intensity():
    """UK National Grid — carbon intensity of electricity generation by region."""
    try:
        ...
        if res.status_code == 200:
            data = res.json()
            regions = []
            for r in data.get("data", [{}])[0].get("regions", []):
                intensity = r.get("intensity", {})
                regions.append({
                    "id": r.get("regionid"),
                    "name": r.get("shortname"),
                    "intensity": intensity.get("forecast"),
                    "index": intensity.get("index"),  # very low/low/moderate/high/very high
                    "generation_mix": r.get("generationmix", []),
                })
            latest_data["carbon_intensity"] = {
                "regions": regions,
                "from": data.get("data", [{}])[0].get("from"),
                "to": data.get("data", [{}])[0].get("to"),
            }
            logger.info(f"Fetched carbon intensity for {len(regions)} UK regions")
    except Exception as e:
...
def fetch_submarine_cables():
    ...
    cables = []
    try:
        url = "https://raw.githubusercontent.com/telegeography/www.submarinecablemap.com/master/web/public/api/v3/landing-point/landing-point-geo.json"
        res = fetch_with_curl(url, timeout=15)
        if res.status_code == 200:
            data = res.json()
            for feat in data.get("features", []):
                ...
                cables.append({
                    ...
                    "lat": coords[1],
                    "lng": coords[0],
                    "is_active": props.get("is_tbd", "") != "1",
                })
        logger.info(f"Fetched {len(cables)} submarine cable landing points")
    except Exception as e:
...
def fetch_ebird():
    """eBird — notable bird sightings worldwide (requires EBIRD_API_KEY)."""
    sightings = []
    api_key = os.environ.get("EBIRD_API_KEY", "")
    if not api_key:
        logger.warning("EBIRD_API_KEY not set, skipping eBird fetch")
        return
    try:
        # Notable observations worldwide — returns recent noteworthy sightings
        # Fetch from multiple hotspot regions for global coverage
        regions = ["US", "GB", "DE", "AU", "BR", "ZA", "IN", "JP", "MX", "KE"]
        def _fetch_region(region_code):
            url = f"https://api.ebird.org/v2/data/obs/{region_code}/recent/notable?maxResults=30"
            headers = {"X-eBirdApiToken": api_key}
            try:
                res = fetch_with_curl(url, timeout=10, headers=headers)
                if res.status_code == 200:
                    return res.json()
            except Exception:
                pass
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as pool:
            results = list(pool.map(_fetch_region, regions))
        ...
        logger.info(f"Fetched {len(sightings)} notable eBird sightings")
    except Exception as e:
        logger.error(f"Error fetching eBird: {e}")
    latest_data["ebird_sightings"] = sightings


def fetch_purpleair():
    """PurpleAir — outdoor air quality sensors (requires PURPLEAIR_API_KEY)."""
    sensors = []
    api_key = os.environ.get("PURPLEAIR_API_KEY", "")
    if not api_key:
        logger.warning("PURPLEAIR_API_KEY not set, skipping PurpleAir fetch")
        return
    try:
        # Fetch outdoor sensors with PM2.5 readings (fields: name, lat, lon, pm2.5)
        url = "https://api.purpleair.com/v1/sensors?fields=name,latitude,longitude,pm2.5_10minute,humidity,temperature&location_type=0&max_age=3600&nwlat=60&nwlng=-130&selat=-40&selng=150"
        headers = {"X-API-Key": api_key}
        res = fetch_with_curl(url, timeout=20, headers=headers)
        if res.status_code == 200:
            data = res.json()
            fields = data.get("fields", [])
            for row in data.get("data", [])[:1500]:
                entry = dict(zip(fields, row))
                lat = entry.get("latitude")
                lng = entry.get("longitude")
                pm25 = entry.get("pm2.5_10minute")
                if lat is None or lng is None or pm25 is None:
                    continue
                sensors.append({
                    "lat": lat,
                    "lng": lng,
                    ...
                    "temp_f": entry.get("temperature"),
                    "aqi": _pm25_to_aqi(pm25),
                })
        logger.info(f"Fetched {len(sensors)} PurpleAir sensors")
    except Exception as e:
        logger.error(f"Error fetching PurpleAir: {e}")
    latest_data["purpleair"] = sensors
...
def fetch_alienvault_otx():
    """AlienVault OTX — latest cyber threat pulses (requires ALIENVAULT_OTX_KEY)."""
    pulses = []
    api_key = os.environ.get("ALIENVAULT_OTX_KEY", "")
    if not api_key:
        logger.warning("ALIENVAULT_OTX_KEY not set, skipping OTX fetch")
        return
    try:
        url = "https://otx.alienvault.com/api/v1/pulses/subscribed?page=1&limit=30"
        headers = {"X-OTX-API-KEY": api_key}
        res = fetch_with_curl(url, timeout=15, headers=headers)
        if res.status_code == 200:
            data = res.json()
            for p in data.get("results", [])[:30]:
                ...
                pulses.append({
                    ...
                    "tags": tags[:10],
                    "targeted_countries": targeted_countries[:10],
                    "ioc_count": ioc_count,
                    "adversary": p.get("adversary", ""),
                    "tlp": p.get("tlp", ""),
                    "malware_families": [m.get("display_name", "") for m in p.get("malware_families", [])][:5],
                })
        logger.info(f"Fetched {len(pulses)} OTX threat pulses")
    except Exception as e:
        logger.error(f"Error fetching AlienVault OTX: {e}")
    latest_data["cyber_threats"] = pulses


def fetch_ticketmaster():
    """Ticketmaster Discovery — upcoming events near major cities (requires TICKETMASTER_KEY)."""
    events = []
    api_key = os.environ.get("TICKETMASTER_KEY", "")
    if not api_key:
        logger.warning("TICKETMASTER_KEY not set, skipping Ticketmaster fetch")
        return
    try:
        # Fetch events across several large cities
        geopoints = [
            "40.7128,-74.0060",  # NYC
        ...
        def _fetch_geo(latlon):
            url = f"https://app.ticketmaster.com/discovery/v2/events.json?apikey={api_key}&latlong={latlon}&radius=100&unit=km&size=20&sort=date,asc"
            try:
                res = fetch_with_curl(url, timeout=10)
                if res.status_code == 200:
                    data = res.json()
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as pool:
            results = list(pool.map(_fetch_geo, geopoints))
        ...
        logger.info(f"Fetched {len(events)} Ticketmaster events")
    except Exception as e:
        logger.error(f"Error fetching Ticketmaster: {e}")
    latest_data["events"] = events


def fetch_global_fishing():
    """Global Fishing Watch — recent fishing vessel activity (requires GLOBAL_FISHING_WATCH_TOKEN)."""
    vessels = []
    token = os.environ.get("GLOBAL_FISHING_WATCH_TOKEN", "")
    if not token:
        logger.warning("GLOBAL_FISHING_WATCH_TOKEN not set, skipping GFW fetch")
        return
    try:
        # Use the 4Wings API to get fishing activity heatmap tiles
        # For individual vessel positions, use the vessels endpoint
        ...
        url = f"https://gateway.api.globalfishingwatch.org/v3/4wings/report?spatial-resolution=low&temporal-resolution=daily&group-by=flag&datasets[0]=public-global-fishing-effort:latest&date-range={start_str},{end_str}&format=json"
        headers = {"Authorization": f"Bearer {token}"}
        res = fetch_with_curl(url, timeout=20, headers=headers)
        if res.status_code == 200:
            data = res.json()
            ...
            logger.info(f"Fetched {len(vessels)} GFW fishing activity entries")
        else:
            logger.warning(f"GFW returned status {res.status_code}: {res.text[:200]}")
    except Exception as e:
        logger.error(f"Error fetching Global Fishing Watch: {e}")
    latest_data["fishing_activity"] = vessels


def fetch_newsapi():
    """NewsAPI — top headlines from 150K+ sources (requires NEWSAPI_KEY)."""
    articles = []
    ...
    latest_data["newsapi"] = articles


def fetch_finnhub_market():
    """Finnhub — real-time market movers and economic calendar (requires FINNHUB_API_KEY)."""
    market_data = {}
    ...
    try:
        ...
        res = fetch_with_curl(url, timeout=10)
        if res.status_code == 200:
            market_data["us_market_status"] = res.json()

        # Crypto symbols — BTC and ETH
        for symbol in ["BINANCE:BTCUSDT", "BINANCE:ETHUSDT"]:
        ...
        logger.info("Fetched Finnhub market data")
    except Exception as e:
        logger.error(f"Error fetching Finnhub: {e}")
    latest_data["finnhub"] = market_data
...
def update_slow_data():
    ...
    slow_funcs = [
        ...
        fetch_ticketmaster,
        fetch_global_fishing,
        fetch_newsapi,
        fetch_finnhub_market,
    ]
...
def start_scheduler():
    ...
    # Liveuamap disabled — Playwright removed to save memory
